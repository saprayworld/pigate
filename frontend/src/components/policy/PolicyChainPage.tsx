...
import { type PolicyChain, type PolicyRule, type AddressObject, type ServiceObject, type NetworkInterface } from "@/data-mockup/mockData"
...
// PolicyChainPage is the shared implementation behind all three policy pages
// (Firewall Policy / Local-In Policy / Local-Out Policy — see
// docs/ref/todo/input-output-chain-firewall-plan.md T-13). The three pages
// under src/pages are thin wrappers that just pick a chain/title/description;
// all table/drawer/drag-reorder logic lives here so a fix only needs to
// happen once.
...
export interface PolicyChainPageProps {
  /** Which nftables base chain this page manages. */
  chain: PolicyChain
  /** Page/card title, e.g. "Firewall Policy" / "Local-In Policy" / "Local-Out Policy". */
  pageTitle: string
  /** Short description under the title explaining what the chain governs. */
  pageDescription: string
}

export default function PolicyChainPage({ chain, pageTitle, pageDescription }: PolicyChainPageProps) {
  const { alert, confirm } = useAlert()
  const showInInterface = chain !== "output"
  const showOutInterface = chain !== "input"
  // NAT only makes sense for traffic actually routed through the box.
  const showNat = chain === "forward"
  ...
  const handleApplySettings = async () => {
    ...
    try {
      // Apply always re-applies all 3 chains (forward/input/output) together,
      // regardless of which page the user clicks this from.
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    try {
      if (editingRule) {
        await policyService.update(editingRule.id, payload)
      } else {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        await policyService.saveAll(chain, newRules)
      } catch (err) {
    ...
    <div className="space-y-4">
      ...
      {chain === "input" && (
        <Alert>
          ...
          <AlertTitle className="font-semibold">Admin Access ถูกประเมินก่อนกฎเหล่านี้เสมอ</AlertTitle>
          <AlertDescription className="text-xs text-muted-foreground">
            สิทธิ์เข้าถึงจากหน้า <strong className="text-foreground">Interfaces</strong> (HTTP/HTTPS/SSH/PING ต่ออินเทอร์เฟซ)
            ถูกอนุญาตก่อนกฎ Local-In เสมอ — กฎ DROP ที่สร้างที่นี่จึงไม่สามารถล็อกคุณออกจากหน้าเว็บ/SSH ของตัวเองได้เชิงโครงสร้าง
            หากต้องการจำกัด SSH ให้เฉพาะบางซับเน็ต ให้ปิด SSH ใน Admin Access ของอินเทอร์เฟซนั้นก่อน แล้วค่อยเขียนกฎ ACCEPT แบบแคบที่นี่แทน
          </AlertDescription>
        </Alert>
      )}
      {chain === "output" && (
        <Alert>
          <Info className="h-4 w-4" />
          <AlertTitle className="font-semibold">ค่าเริ่มต้นคืออนุญาตทั้งหมด (Default Allow)</AlertTitle>
          <AlertDescription className="text-xs text-muted-foreground">
            Local-Out Policy ใช้ policy accept — ทราฟฟิกขาออกจากตัวบอร์ดที่ไม่ตรงกับกฎ DROP ใดๆ ที่นี่จะผ่านได้ตามปกติ
            (ยกเว้นทราฟฟิก IPv6 ซึ่งถูกบล็อกไว้เป็นค่าเริ่มต้นเพื่อความปลอดภัย)
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="w-full overflow-x-auto">
            <Table>
              ...
              <TableBody>
                ...
                {/* Default Implicit Row — DROP for forward/input (policy drop),
                    ACCEPT for output (policy accept). */}
                <TableRow className="border-t border-border bg-muted/30 text-muted-foreground hover:bg-muted/30">
                  ...
                  <TableCell className="py-3 text-sm font-medium italic">
                    {chain === "output" ? "Implicit Allow (ACCEPT)" : "Implicit Deny (DROP)"}
                  </TableCell>
                  ...
                  <TableCell className="py-3">
                    {chain === "output" ? (
                      <Badge variant="outline" className="rounded border-primary/20 bg-primary/10 px-2 py-0.5 text-[10px] font-bold text-primary">
                        ACCEPT
                      </Badge>
                    ) : (
      ...
      <Drawer direction="right" open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        >
          ...
          <form onSubmit={handleSaveForm} className="space-y-4 text-sm">
            ...
            {/* Row 1.5: In Interface & Out Interface — In hidden for output
                chain, Out hidden for input chain (Chain field only ever
                matches on the meaningful direction). */}
            {(showInInterface || showOutInterface) && (
              <div className="grid gap-4 sm:grid-cols-2">
                {showInInterface && (
                  <div className="space-y-1.5">
                    <Label className="block text-xs font-medium text-muted-foreground">
