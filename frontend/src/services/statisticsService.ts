...
export interface TopConversation {
  ...
  bytes: number;
  percent: number;
  // bytesUp = bytes sent srcIp -> dstIp (orig direction), bytesDown = bytes
  // sent dstIp -> srcIp (reply direction). bytesUp + bytesDown == bytes.
  bytesUp: number;
  bytesDown: number;
...
// upRatio mirrors kernel/mock.go's mockFlowTemplate.upRatio (T-04): the
...
const mockDests = [
  ...
  { ip: "8.8.8.8", hostname: "8.8.8.8", upRatio: 0.55 }, // DNS: roughly balanced
  { ip: "64.233.166.127", hostname: "64.233.166.127", upRatio: 0.5 }, // VoIP/SIP: symmetric
];
...
function mockTopConversations(scale: number): TopConversation[] {
  const raw = [
    { srcIp: "192.168.1.102", srcHostname: "Android-SmartTV", dstIp: "173.194.76.94", dstHostname: "173.194.76.94", proto: "TCP", dstPort: 443, bytes: 26_000_000 * scale, upRatio: 0.08 },
    { srcIp: "192.168.1.101", srcHostname: "iPhone-13", dstIp: "142.250.80.46", dstHostname: "142.250.80.46", proto: "TCP", dstPort: 443, bytes: 9_000_000 * scale, upRatio: 0.15 },
    { srcIp: "192.168.1.105", srcHostname: "iPad-Pro", dstIp: "151.101.1.69", dstHostname: "151.101.1.69", proto: "TCP", dstPort: 443, bytes: 4_500_000 * scale, upRatio: 0.15 },
    { srcIp: "192.168.1.105", srcHostname: "iPad-Pro", dstIp: "151.101.1.69", dstHostname: "151.101.1.69", proto: "TCP", dstPort: 80, bytes: 3_000_000 * scale, upRatio: 0.3 },
    { srcIp: "192.168.1.102", srcHostname: "Android-SmartTV", dstIp: "64.233.166.127", dstHostname: "64.233.166.127", proto: "UDP", dstPort: 5060, bytes: 1_200_000 * scale, upRatio: 0.5 },
  ];
  const total = raw.reduce((sum, r) => sum + r.bytes, 0);
  return raw.map((r) => {
    const bytesUp = Math.round(r.bytes * r.upRatio);
    return { ...r, percent: Math.round((r.bytes / total) * 1000) / 10, bytesUp, bytesDown: r.bytes - bytesUp, dstDomain: mockIpDomains[r.dstIp] ?? "" };
  });
}
