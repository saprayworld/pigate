...
| **Multi-WAN Failover** | Completed | Completed | **IPv4 only.** Per-uplink health monitoring (ICMP and/or TCP-connect probes, with `auto` ICMP→TCP fallback within the same round when ICMP is blocked/dead) drives an up/degraded/down state machine — "degraded" (latency over threshold, no loss) is display-only and never triggers a failover. A Phase 2 controller (default **off** — kill switch) can then automatically move the default route between uplinks by priority, with anti-flap dampening (`minHoldSeconds`) and a revert delay before switching back to a recovered higher-priority uplink; a manual override can also force a specific uplink active. Enforcement is entirely a fixed default-route metric band (active uplink gets a low/"winning" metric, standby uplinks get a high one, ordered by priority) applied via Netlink — never `fwmark`, policy routing, or a second routing table. An active Static Route for `0.0.0.0/0` on the same interface always takes precedence over the failover override (surfaced in the UI as "bypassed"). Kill switch and manual override are `super_admin`-only. |
| **DNS System** | Completed | Completed | `systemd-resolved` per-link DNS configuration via D-Bus. |
| **Firewall System** | Completed | Completed | `nftables` management via Netlink; policy configuration across all three base chains — Firewall Policy (`forward`, traffic passing through the box), Local-In Policy (`input`, traffic to the box itself), Local-Out Policy (`output`, traffic from the box itself, default-allow) — plus WAN Network Address Translation (Masquerade) and Docker compatibility. Address Objects and Service Objects each support **multiple entries per name** (e.g. several subnet/range/fqdn values, or several protocol/port pairs, under one object) — a policy rule referencing such an object expands into one nft rule per entry combination at apply time, all sharing the same rule id so per-rule statistics and matched-endpoint labeling still report as a single rule; entry counts are bounded by the `max-object-entries` and `max-expanded-rules-per-policy` config keys (see the Runtime settings section below). |
...
| **DNS Blocklists** | Completed | Completed | Bulk blocklist import ("Blocklists" tab on the DNS Server page) — subscribe to a public hosts-format URL (fetched and re-fetchable via Refresh) or upload a hosts file directly, up to 8 lists / 16 MB per file / 500,000 domains total. Each list picks its own block mode: `sinkhole` (default, `addn-hosts=<id>.hosts`, exact-match only) or `nxdomain` (`conf-file=<id>.conf` with `address=/<domain>/` lines, covers subdomains too, requires dnsmasq ≥ 2.86, capped separately at 150,000 domains across all `nxdomain` lists). The parser always discards the source file's own IP column and re-renders `0.0.0.0 <domain>` itself, so a malicious or MITM'd list can never redirect a domain to an arbitrary address. Metadata (name, source, mode, domain count, last fetch, errors) lives in a single JSON manifest (`/var/lib/pigate/blocklists/manifest.json`), not SQLite — the generated `.hosts`/`.conf` files are the only place the domain list itself is held. Separate from, and does not replace, the original per-domain "Blocked Domains" deny-list; see `docs/ref/complete/dns-system-design.md` ("Blocklists (bulk import)") for the full design. |
...
| **Statistics → Firewall** | Completed | Completed | Statistics ▸ Firewall page (`/statistics/firewall`, docs/ref/todo/statistics-firewall-page-plan.md): summary cards (accepted/blocked bytes+packets, blocked events, enabled/unused rule counts), an accept-vs-drop bytes trend chart and a separate blocked-events trend chart, a Chain Breakdown table (forward/input/output), a Top Rules by Traffic table that links into the matching Policy page, full Top Blocked Sources/Ports lists, and a Recent Blocked Events table with a link out to the Forward/Local Traffic log (pre-filtered to DROP). Two independently-sourced, differently-scoped accountings are combined and never mixed into one percentage: `acceptedBytes`/`blockedBytes` come from the exact nft per-rule counter (the same source `/policies/stats` uses) but only ever cover traffic that matched a user-defined policy rule — traffic dropped by the board's built-in default-drop is NOT included, called out via an info tooltip on every card that shows it — while `blockedEvents` (and the blocked-events trend/sources/ports/recent-events tables) come from NFLOG and DO include default-drop traffic. Built entirely on existing RAM-only rings (the traffic bucket ring's per-rule counters, the deny ring, and the traffic-log ring buffer) — no new kernel capability, no new goroutine/ticker, no new SQLite table. |
...
### Configuration File
Runtime settings can be changed without editing the systemd unit. `install.sh` creates `/var/lib/pigate/pigate.conf` (a simple `key=value` file, `#` for comments) with production defaults on first install. Most keys are the CLI flag names without the leading `-`; thirteen keys (`dns-stats-max-pairs`, `dns-stats-max-clients`, `dns-stats-max-domains`, `dns-stats-max-ips-per-domain`, `traffic-stats-max-hosts`, `traffic-stats-max-dests`, `traffic-stats-max-conversations`, `ipinfo-enabled`, `deny-stats-max-sources`, `deny-stats-max-ports`, `traffic-log-buffer-capacity`, `max-object-entries`, `max-expanded-rules-per-policy`) are intentionally file-only tuning knobs with no CLI flag counterpart — the three `traffic-stats-max-*` keys bound the Statistics ▸ Traffic page's RAM-only bucket ring (5-minute buckets, 288 deep = 24h); each raised value costs roughly `value × 288 × ~110 bytes` worst case, and `deny-stats-max-sources`/`deny-stats-max-ports` bound the firewall deny ring's per-bucket caps the same way (see `pigate.conf.example` for the exact numbers). `dns-stats-max-domains` (default `1000`, accepted range 100–20000) and `dns-stats-max-ips-per-domain` (default `32`, accepted range 2–64) bound the Statistics ▸ DNS page's `domain → resolved IP` forward index used for the domain volume/drill-down feature — unlike the ring-based keys above, this index is **not** a ring (it holds current knowledge, not per-bucket history), so its worst-case RAM is just `maxDomains × maxIPsPerDomain` entries (≈2 MB at the defaults), with no bucket-count multiplier. As with the other file-only keys, an out-of-range value is clamped back to its default with a startup warning rather than failing to boot. Note: `dns-stats-max-ips-per-domain`'s default changed from `16` to `32` — a machine whose `/var/lib/pigate/pigate.conf` was auto-generated by the binary itself (not by `install.sh`, which never seeds this key) before that change will still have `dns-stats-max-ips-per-domain=16` written in the file, and config file always wins over the code default, so it will keep using `16` until that line is edited or removed by hand. `ipinfo-enabled` (default `false`) is a plain opt-in switch, not a RAM-guard knob — see the privacy note below. `traffic-log-buffer-capacity` (default `10000`, accepted range 500–100000) sizes the shared traffic log ring buffer that backs the Forward/Local Traffic log pages and the Statistics ▸ Capacity "Firewall Traffic Log Buffer" card — it is only read once at process startup, so a change here needs `sudo systemctl restart pigate` to take effect. `max-object-entries` (default `64`, accepted range 1–512) caps how many entries (e.g. subnet/range/fqdn values, or protocol/port pairs) a single Address Object or Service Object may hold, and `max-expanded-rules-per-policy` (default `4096`, accepted range 64–65536) caps how many nft rules a single firewall policy may expand into — both back Multi-Value Address & Service Objects, are only read once at process startup (same restart-required caveat as `traffic-log-buffer-capacity`), and use the same clamp-with-warning out-of-range validation as the other file-only keys. See [`pigate.conf.example`](pigate.conf.example) in the repo root for a fully commented sample of every key, its default, and what it does.

**Privacy note — Public IP Info (`ipinfo-enabled`):** the Statistics ▸ Traffic ▸ Host drill-down page can show a "Public IP Info" card (city/region/country/org) for public destination IPs, backed by a gateway-side proxy to the third-party service [ipinfo.io](https://ipinfo.io) (no API token in the current phase — the free, unauthenticated tier). This is **off by default** and can only be turned on by hand-editing `/var/lib/pigate/pigate.conf` and setting `ipinfo-enabled=true` — there is no UI toggle. When enabled, only the specific public IP being looked up is sent to ipinfo.io (never LAN addresses, never the admin browser's own IP — the request originates entirely from the gateway), results are cached in RAM only (never written to SQLite, and reset on restart) and rate-limited to protect ipinfo.io from the page's own auto-refresh.
