...
func setupTestServer(t *testing.T) (http.Handler, *db.Repository) {
	...
	server := NewServer(repo, fw, net, rt, dhcp, ringBuffer, false, ifaceService, routingService, fwService, dnsService, qosService)
...
func TestDisableEditMode(t *testing.T) {
	...
	server := NewServer(repo, fw, net, rt, dhcp, ringBuffer, true, ifaceService, routingService, fwService, dnsService, qosService)
	handler := RegisterRoutes(server)
