...
>
> **อัปเดต 2026-07-28 (ก่อนเริ่ม implement):** ตรวจสอบซ้ำแล้ว ยังไม่มี commit/PR ใดแตะฟีเจอร์นี้
> โค้ดทุก layer ยังตรงกับที่แผนสำรวจไว้ 100% — ใช้แผนนี้ได้ตรงๆ ไม่ต้องวางแผนใหม่ มีแค่เลขบรรทัด
> ใน `api/handlers.go` และ `service/firewall.go` ที่ขยับจาก commit หลังวันที่เขียนแผน (ไม่กระทบ
> เนื้อหา/ลำดับ dependency): **create policy handler อยู่ที่ `:1060`** (แผนเขียน `:1042`),
> **update handler `:1098`** (แผนเขียน `:1080`), **reorder handler `:1137`** (แผนเขียน `:1119`),
> **`SyncFirewallRules` อยู่ที่ `firewall.go:136`** (แผนเขียน `:120`) — ให้ยึดชื่อฟังก์ชัน/หา
> ด้วย grep แทนเลขบรรทัดตรงๆ เสมอ เพราะโค้ดอาจขยับอีกก่อนเริ่มแก้จริง
