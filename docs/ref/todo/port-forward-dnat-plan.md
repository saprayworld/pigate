...
- DNAT แบบ 1:1 (map ทั้ง IP), load-balance หลาย backend, port-range translation ที่ไม่ 1:1
...
// ต่อ port-forward entry: iifname==<ext> && <proto> dport==<extPort> → dnat to internalIP:internalPort
//   expr.Meta{IIFNAME} + Cmp, expr.Payload(proto+dport) + Cmp, expr.Immediate(ip+port), expr.NAT{Type: DNAT}
```
...
- real: เพิ่ม prerouting chain + DNAT rules + auto-gen forward-accept (ก่อน final drop)
...
1. **[กับดักคลาสสิก] DNAT ถูกแต่ firewall drop** — หลัง DNAT dst=internal → ต้องผ่าน forward chain.
   *เกิดอะไร:* ไม่มี forward-accept = port-forward "ดูเหมือนตั้งถูก" แต่ยิงไม่ทะลุ. *กัน:* auto-gen
   forward-accept คู่ทุก entry (Step 2), วางก่อน final-drop-log ของ forward chain
...
- [ ] `model/types.go` — `PortForward` + `PortForwardInput` + validation helper (IP/port/proto)
- [ ] `kernel/interfaces.go` + `real_firewall.go` + `mock.go` — `ApplyRules` รับ port-forwards;
      real เพิ่ม prerouting DNAT + auto forward-accept; mock log-only
