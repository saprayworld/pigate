...
- [ ] เลือกแนวทาง A หรือ B (แนะนำ B ถ้าต้องการให้ Settings ↔ sidebar sync)
- [ ] ลบ string hardcode `home-000002.pigate.lan` ออกจาก `app-sidebar.tsx`
- [ ] ดึง hostname จริงจาก `systemService.getHostname()` + fallback `pigate`
- [ ] (ถ้าเลือก B) wire หน้า Settings ให้ update ค่าใน context หลังบันทึกสำเร็จ
- [ ] ตัดสินใจเรื่อง suffix `.lan` / FQDN (ตอนนี้แนะนำโชว์ hostname เปล่า)
