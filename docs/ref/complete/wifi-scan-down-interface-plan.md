...
[
  ...
  {
    "task_id": "T-04",
    "title": "[SENSITIVE] กัน TRIGGER_SCAN ซ้อนกันด้วย per-interface singleflight ใน RealNetwork.ScanWifi",
    "layer": "kernel",
    "files": ["backend/internal/kernel/real_network.go", "backend/go.mod", "backend/go.sum"],
    "instruction": "เพิ่มเข้าแผนหลังทดสอบ T-02 บน RPi5 จริงแล้วพบว่า: T-02 รับประกันแค่ 'TRIGGER_SCAN ไม่เกิน 1 ครั้งต่อการเรียก ScanWifi 1 ครั้ง' แต่ HandleScanWifi ไม่มี lock/dedup กัน concurrent HTTP requests — กด Scan รัวๆ (spam) ทำให้แต่ละคลิก spawn goroutine ใหม่เรียก ScanWifi ทั้งชุด เกิด TRIGGER_SCAN ซ้อนกันหลายตัวพร้อมกันบน interface เดียว (เห็นจริงใน journalctl log บนเครื่อง production) ตรงกับสิ่งที่ §0.3 ตั้งใจห้ามไว้ (กวน wpa_supplicant ตอน associate) เพียงแต่ตอนนั้นป้องกันแค่ระดับ 1 call ไม่ได้ป้องกันระดับ concurrent requests. แก้ด้วย golang.org/x/sync/singleflight (มีอยู่แล้วใน go.mod เป็น indirect — เลื่อนเป็น direct ด้วย go mod tidy ไม่ต้องเพิ่ม dependency ใหม่). (1) เพิ่มฟิลด์ scanGroup singleflight.Group ใน struct RealNetwork (zero value ใช้ได้เลย). (2) ย้าย logic เดิมทั้งหมดของ ScanWifi (ตั้งแต่ wifi.New() จนถึง poll loop) ไปเป็นเมธอดแยก เช่น scanWifiOnce(name) โดยไม่แก้เนื้อหาเลย. (3) ScanWifi(name) กลายเป็น wrapper: v, err, _ := r.scanGroup.Do(name, func() (interface{}, error) { return r.scanWifiOnce(name) }); ถ้า err != nil คืน nil, err ก่อน assert เสมอ; ไม่งั้น type-assert v.([]model.WifiScanResult) คืนออกไป. ข้อบังคับ: key ของ Do ต้องเป็น 'name' (ชื่อ interface จริง) ห้ามใช้ค่าคงที่/global เด็ดขาด เพราะจะบล็อก scan ข้าม interface (เช่น wlan0 กับ USB dongle ต้อง scan พร้อมกันได้). ห้ามเขียน map[string]*sync.Mutex เอง — singleflight ลบ key ออกอัตโนมัติเมื่อ call จบ ไม่ leak. ห้ามแตะ mock.go (signature ไม่เปลี่ยน). **SENSITIVE: review ว่า (ก) key เป็น per-interface จริง ไม่บล็อกข้าม interface, (ข) leader มี ctx timeout 10s เดิม → joined callers bounded ตาม ไม่ค้างถาวร, (ค) ไม่มี panic จาก type assertion เมื่อ error/nil, (ง) invariant 'TRIGGER_SCAN ≤ 1 ต่อ scan ที่วิ่งจริง' ยังเป็นจริงแม้มี concurrent callers.**",
    "acceptance": ["go build + go vet + go test ./... ผ่าน", "go test -race -count=1 ./internal/kernel/... ผ่าน", "go mod tidy แล้ว golang.org/x/sync เป็น direct dependency", "อ่านโค้ดยืนยัน key = ชื่อ interface, ไม่มี map[string]*Mutex เขียนเอง", "request ที่ซ้อนกันบน interface เดียว join ผลเดียวกัน ไม่ trigger ใหม่; interface คนละตัวยัง scan พร้อมกันได้"],
    "depends_on": ["T-02"]
  }
]
```
...
```json
{
  "final_acceptance": [
    ...
    "กด Scan รัวๆ (spam) บน interface เดียว → journalctl เห็น 'triggering scan on <iface>' ครั้งเดียวต่อ burst ไม่ใช่ซ้อนกันทุก 1-2s (ยืนยัน T-04 singleflight ทำงาน, ไม่กวน wpa_supplicant associate)",
    "กด Scan พร้อมกันบน 2 interface ต่างกัน (เช่น wlan0 + USB dongle) → ทั้งคู่คืนผลตามปกติ ไม่บล็อกกัน (ยืนยัน key เป็น per-interface)",
    "go test ./... (รวม -race บน internal/kernel) และ yarn build/lint ผ่านทั้งหมด"
  ]
}
```
