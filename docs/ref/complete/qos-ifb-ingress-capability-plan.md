...
- [x] `backend/internal/model/types.go` — เพิ่ม `IngressSupported` ใน `QosIfaceStatus`
- [x] `backend/internal/kernel/real_qos.go` — probe ใน `NewRealQos()` + guard ใน
      `ApplyQosRules` + ลบ `modprobe` + ใส่ field ใน `GetIfaceQosStatus`
- [x] `backend/internal/kernel/mock.go` — `IngressSupported: true`
- [x] `docs/openapi.yaml` + `frontend/public/openapi.yaml` — schema `QosIfaceStatus` (sync ทั้งคู่)
- [x] `frontend/src/services/qosService.ts` — type + mock branch
- [x] `frontend/src/pages/QoS.tsx` — banner + disable ingress inputs + ไอคอน/ขีดฆ่าเตือนในตาราง
- [x] `docs/ref/qos-system.md` — หัวข้อ IFB ingress capability
- [x] Test: `cd backend && go build ./... && go test ./...` ผ่าน (+ `go vet`)
- [x] Test: `cd frontend && yarn build && yarn lint` ผ่าน
- [x] Test (mock, workstation): `-mock=true` → `GET /api/qos/status/{eth0,wlan0}` คืน
      `ingressSupported: true` ผ่าน full HTTP stack (login→change-password→re-login→GET)
- [x] Test (จำลองไม่รองรับ): hardcode `ingressSupported=false` → banner แสดง, ช่อง Ingress
      ถูก disable, edit rule ที่มี ingress > 0 แล้ว save → ค่า ingress ไม่ถูกล้างเป็น 0 (Caution 6) — ยืนยันแล้ว
- [x] Test (บอร์ดจริง มี ifb): boot log ขึ้น `[RealQos] IFB module available, ingress shaping enabled`
      (ไม่มี warning IFB), ingress shaping ทำงาน — ยืนยันบนบอร์ดจริงแล้ว
- [x] Test (role): login ด้วย role read-only → เห็น banner ได้ (GET ไม่ถูก block) — ยืนยันแล้ว
- [x] อัปเดตแผนนี้ถ้าหน้างานต่างจากที่เขียน แล้วย้ายไป `docs/ref/complete/` เมื่อเสร็จ

### Implementation notes (หน้างานจริง vs แผน)
- **หน้า QoS ใช้ `Drawer` (direction=right) ไม่ใช่ `Dialog`** — ช่อง Ingress อยู่ในฟอร์มใน Drawer
  ไม่มี Combobox จึงไม่กระทบ rule `<Dialog modal={false}>`. banner วางที่ต้นสุดของหน้า (section 0)
  เพราะหน้าไม่มี header/title bar แยก
- **Banner ใช้ `<Alert>` (variant `default`) + semantic warning classes** (`border-warning/30`,
  `bg-warning/10`, `text-warning`) แทนการเพิ่ม variant `warning` ใน `alert.tsx` — ไม่แตะ primitive
- ตาราง rule: cell Ingress Limit ขีดฆ่า (`line-through opacity-60`) + ไอคอน `AlertCircle` เมื่อ
  `ingressUnsupported`
- ลบ `execCommand("modprobe","ifb")` แล้ว — `var execCommand` ยังถูกใช้ที่ `real_network.go`/
  `wpa_test.go` จึงไม่มี unused-symbol
