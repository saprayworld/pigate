...
- [~] Test (จำลองไม่รองรับ): banner/disable derive จาก `some(s => s.ingressSupported === false)` —
      ตรรกะ type-checked + build ผ่าน; ทดสอบ visual ด้วย hardcode `false` เป็น manual QA
      ก่อน merge (Caution 6: input `disabled` ยัง hold ค่าจาก state → submit ไม่ล้างเป็น 0)
- [ ] Test (บอร์ดจริง มี ifb): boot แล้ว log ไม่มี warning IFB, ingress shaping ยังทำงาน
      (`tc -s qdisc show dev ifb-<iface>` เทียบก่อน/หลัง), ไม่มี link `pigate-ifb0` ค้าง — **board-only**
- [ ] Test (role): login ด้วย role read-only → เห็น banner ได้ (GET ไม่ถูก block) — manual QA
