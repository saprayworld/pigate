...
| 6 | กลาง | ไม่มี security headers (`Content-Security-Policy`, `X-Frame-Options`, `X-Content-Type-Options`) บนหน้า SPA | `embed.go`/middleware | เพิ่ม header middleware; CSP ช่วยลดผลของข้อ 2 ด้วย |
| 7 | ต่ำ–กลาง | `SendWpaCommand` log command เต็ม อาจรั่ว PSK ใน journal หากอนาคตส่ง command ที่มีรหัสผ่าน | `kernel/wpa.go` | redact ก่อน log |
| 8 | ต่ำ | `_, _ = rand.Read(b)` เพิกเฉย error ตอนสร้าง token | `handlers.go:117` | จัดการ error |
| 9 | ต่ำ | CORS อนุญาต origin dev (`localhost:5173`) แม้ใน production binary | `middleware.go` | เปิดเฉพาะเมื่อมี flag dev |
| 10 | ต่ำ | ไม่มี audit log ว่าใครแก้ config อะไรเมื่อไร (มีแต่ firewall event ring buffer) | service layer | เพิ่ม audit ring buffer แยก บันทึก username+action |
