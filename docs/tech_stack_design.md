...
* **Solution**:
  ...
  * **Dashboard traffic analytics** (แท็บ Detailed — Protocol Breakdown / Top Talkers / Top Rules by Traffic, `service/traffic_stats.go`) ก็อยู่ใน RAM ring buffer เดียวกันแบบเดียวกับ `TrafficBucket` เดิม (bucket 5 นาที × สูงสุด 288 = 24 ชม.) ไม่เขียนลง SQLite เช่นกัน ข้อมูลมาจากสองแหล่งที่มีลักษณะต่างกัน:
    - ทั้งสองเส้นทาง (poll และ DESTROY event) เก็บ byte แยกทิศทาง **orig/reply**
      ของ conntrack ตั้งแต่ชั้น kernel (`model.FlowSample.BytesOrig`/`BytesReply`)
      ไม่ใช่ก้อนรวมเดียว — หน้า Statistics (`/logs/statistics`) ใช้มิตินี้แสดง
      Upload/Download แยกกันต่อ host/destination/conversation โดยพลิกทิศให้
      Top Destinations (ดู `docs/ref/todo/statistics-split-upload-download-bytes-plan.md`
      §2.5); การ์ด Dashboard "Detailed" ยังคงรวมเป็น byte เดียวเหมือนเดิม
    - **conntrack polling** (`kernel.TrafficAccountingManager.DumpFlows`, ผ่าน `vishvananda/netlink` ทุก 10 วินาที) ให้ byte/host/protocol เป็น **ค่าประมาณการ** (~95–99% ของ byte จริงในสภาพบ้าน/ออฟฟิศทั่วไป — ดู `docs/ref/todo/dashboard-traffic-detail-plan.md` §2.4) เพราะเป็นการ snapshot ตารางเป็นระยะ ไม่ใช่การนับที่ต้นทาง จึงพลาด byte ของ flow ที่เกิด-ตายเร็วมากในหน้าต่าง poll เดียว (พบไม่บ่อยเพราะ timeout ของ conntrack entry เอง ทำให้ flow เกือบทั้งหมดถูกเห็นอย่างน้อยหนึ่งครั้งหลังจบไปแล้ว) — จำเป็นต้องเปิด sysctl `net.netfilter.nf_conntrack_acct=1` (ตั้งให้แล้วใน `install.sh`) ไม่งั้น byte counter จะเป็น 0 ทั้งตาราง
    - **Phase 2 — hybrid poll + conntrack DESTROY event** (`kernel.TrafficAccountingManager.WatchFlowEnd`, `internal/kernel/real_conntrack_events.go`, subscribes `NFNLGRP_CONNTRACK_DESTROY` ด้วย `vishvananda/netlink/nl` โดยตรง): ปิดช่องว่างของ flow ที่เกิด-ตายเร็วกว่ารอบ poll (scan/DNS burst/P2P) ด้วยการรับ byte ก้อนสุดท้ายของทุก flow ตอน kernel ลบทิ้ง แล้ว **เครดิตเฉพาะส่วนต่างจาก baseline ที่ poll เห็นล่าสุด** เข้า bucket เดียวกัน (กัน double count) — เมื่อ listener ทำงานอยู่ `TrafficDetail.accuracy` จะเป็น `"near-exact"` แทน `"estimated"`; ถ้า kernel ไม่รองรับ (`CONFIG_NF_CONNTRACK_EVENTS`) หรือไม่มี `cap_net_admin` ระบบ degrade กลับไป poll อย่างเดียวเงียบๆ (capability id `conntrack-events` จะขึ้น degraded ใน System Capabilities panel) จำเป็นต้องเปิด sysctl `net.netfilter.nf_conntrack_events=1` (ตั้งให้แล้วใน `install.sh`) ไม่งั้น event จะไม่ถูกส่งออกมาเลย — รายละเอียดดู `docs/ref/todo/traffic-accounting-accuracy-phase2-plan.md`
    - **nftables per-rule counter** (`kernel.TrafficAccountingManager.DumpRuleCounters`, อ่าน `expr.Counter` ที่ติดอยู่กับทุก policy rule อยู่แล้ว ผูกกลับเข้ากับ DB rule id ผ่าน `Rule.UserData`) ให้ byte ต่อ rule เป็น **ค่าที่แม่นยำ 100%** เพราะ kernel นับเองตั้งแต่ต้นทาง ไม่ใช่การประมาณ
    - เหตุผลที่ไม่ persist: เหมือน log ring buffer ด้านบนทุกประการ (ถนอมรอบเขียน SD card) — ยอมรับว่าประวัติ traffic ต่อ host/protocol หายเมื่อ reboot
  * **Reference popover** (hover ที่ IP/Domain ในหน้า Logs/Policy/Statistics เห็นสรุป domain↔IP + Public IP Info — `docs/ref/todo/reference-popover-plan.md`, `GET /api/statistics/reference/{ip,domain}`) อ่านจาก RAM-only index/ring เดิมทั้งหมด (`dns_domain_ips.go`'s forward/reverse index, DNS query-count ring) ไม่มี index/ring ใหม่, ไม่มีการเขียนลง SQLite เพิ่ม — **ข้อมูล domain↔IP เป็น display-only และ poisonable ได้โดย LAN client ที่ query domain ที่ตัวเองคุมได้ ห้ามใช้ผลจาก endpoint นี้ (หรือ popover ที่แสดงผล) ไปตัดสิน firewall rule generation, policy matching, routing, หรือ QoS ใด ๆ** เหมือนกับทุก endpoint DNS statistics อื่นในหมวดนี้
...
* **Traffic Accounting — Dashboard traffic analytics (`internal/kernel/real_traffic_account.go`)**: อ่านสถิติ traffic **โดยไม่มี** exec/shell เช่นกัน — `DumpFlows` dump ตาราง conntrack (IPv4+IPv6) ผ่าน `vishvananda/netlink` และ `DumpRuleCounters` อ่าน `expr.Counter` ของทุก policy rule ผ่าน `google/nftables`; เป็น read-only ล้วน (ไม่สร้าง/แก้ rule หรือ conntrack entry ใดๆ) ดูรายละเอียดที่หัวข้อ 8
